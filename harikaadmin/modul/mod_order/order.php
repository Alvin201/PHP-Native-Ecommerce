...
switch($_GET[act]){
  ...
  case "detailorder":
    $edit = mysql_query("SELECT * FROM orders  INNER JOIN kustomer ON  orders.id_kustomer = kustomer.id_kustomer  JOIN konfirmasi ON konfirmasi.id_orders=orders.id_orders WHERE orders.id_orders='$_GET[id]'");
...
echo "<a href='../buktibayar/".$r['bukti_bayar']."' >Download Bukti Bayar</a>";  
    }
    ...
    echo "<h2>Detail Order</h2>
          ...
          <tr><td>Tgl. Bayar</td> <td> : $tanggal & $r[status_bayar]</td></tr>
  ...
  while($s=mysql_fetch_array($sql2)){
   ...
   $totalberat  = $totalberat + $subtotalberat; // grand total berat all produk yang dibeli
    ...
    echo "<tr><td>$s[nama_album]</td><td align=center>$s[berat]</td><td align=center>$s[jumlah]</td>
              <td align=right>$harga <span style='color:red;'>(-$s[diskon]%)</span> </td><td align=right>$subtotal_rp</td></tr>";
  }
